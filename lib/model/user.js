...
User.listTweets = function(id, fn) {
  client.zrevrange('user:' + id + ':tweets', 0, -1, function(err, tweetIds) {
    if (err) {
      return fn(err);
    }
...
/**
 * Get the cardinality of the tweets in timeline.
 *
 * @param  {String}   id - User ID
 * @param  {Function} fn - Callback gets two arguments `(err, cardinality)`
 * where `cardinality` is the number of tweets in the timeline
 * @public
 */
User.getTweetCountInTimeline = function(id, fn) {
  client.zcard('user:' + id + ':timeline', fn);
};
...
User.isFollowing = function(id1, id2, fn) {
  User.listFollowerIds(id1, function(err, followerIds) {
    if (err) {
      return fn(err);
    }
